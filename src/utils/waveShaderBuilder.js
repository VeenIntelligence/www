...
// ══════════════════════════════════════════════════════════════
// Shader 构建器
// 负责将 JS 配置对象转换为 GLSL #define 指令，
// 然后拼接到 fragment shader 主体前面。
// ══════════════════════════════════════════════════════════════

/** 将数字格式化为 GLSL 浮点数字符串。例: 0.5 → "0.5", 1 → "1.0" */
function glslFloat(value) {
  const trimmed = Number(value).toFixed(4).replace(/0+$/, '').replace(/\.$/, '');
  return trimmed.includes('.') ? trimmed : `${trimmed}.0`;
}

/** 将 [r, g, b] 数组格式化为 GLSL vec3。例: [0.2, 0.3, 0.4] → "vec3(0.2, 0.3, 0.4)" */
...
// 配置 → GLSL #define 映射表
//
// 每条 [defineName, valueAccessor] 表示一个映射关系。
// 与原来 90 行手写的效果完全等价，但用声明式数据驱动。
// 要加新参数只需追加一行。
// ══════════════════════════════════════════════════════════════
...
// [GLSL define 名, 值, 类型]
// 类型: 'f' = float, 'v3' = vec3, 'min' / 'max' = 数组取下标
const LOOK_MAP = [
  // ── background ──
...
/**
 * 构建完整的 fragment shader 字符串。
 * @param {'high'|'medium'|'low'} tier - GPU 性能档位
 * @returns {string} 完整的 GLSL fragment shader（含 #define 头）
 */
export function buildFragmentShader(tier) {
